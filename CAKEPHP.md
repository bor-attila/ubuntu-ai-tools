...
* Always use `Cake\ORM\Query\SelectQuery` for find/select operations. Do not typehint the deprecated `Cake\ORM\Query` class for fetching data.
...
* When you pass a `Cake\ORM\Query\SelectQuery` instance to view you don't need to fetch the data - leave it to be evaluated lazily
* Never call `->toArray()` or `->all()` on a query inside a Controller unless explicit array manipulation is required before rendering. Let the View layer evaluate it lazily.
### Baking
...
* ALWAYS use the `--stop-on-failure` flag during development to prevent terminal log flooding.
* If a specific test fails, run ONLY that test file (e.g., `vendor/bin/phpunit tests/TestCase/Controller/ArticlesControllerTest.php`) instead of the whole suite.
### Extendibility
